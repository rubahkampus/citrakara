...
/**
 * Find and auto-accept all expired milestone uploads across contracts
 * This is useful for handling uploads that should have been auto-accepted
 * but weren't due to timing or server limitations
 *
 * @returns Object containing arrays of processed uploads and errors
 */
export async function processExpiredMilestoneUploads(): Promise<{
  processed: Array<{
    uploadId: string;
    contractId: string;
    milestoneIdx: number;
  ...
  try {
    // Find all milestone uploads that are:
    // 1. In "submitted" status
    // 2. Are final milestone uploads (isFinal: true)
    // 3. Have passed their expiration date
    const currentTime = new Date();

    // Use the ProgressUploadMilestone model directly
    const expiredUploads = await ProgressUploadMilestone.find({
      status: "submitted",
      isFinal: true,
    ...
    for (const upload of expiredUploads) {
      try {
        // Create a new session for each upload to ensure isolation
        const session = await startSession();
        ...
        console.error(`Error processing expired upload ${upload.id}:`, error);
        errors.push({
          uploadId: upload.id.toString(),
          contractId: upload.contractId.toString(),
    ...
    console.error("Error in processExpiredMilestoneUploads:", error);
  }

  return { processed, errors };
}

// Helper function to check a specific contract for expired milestone uploads
export async function processExpiredMilestoneUploadsForContract(
  ...
  try {
    // Find all milestone uploads for this contract that are:
    // 1. In "submitted" status
    // 2. Are final milestone uploads (isFinal: true)
    // 3. Have passed their expiration date
    const currentTime = new Date();
