...
export async function createTransaction({
  ...
  try {
    ...
    const updateField = target === "available" ? "saldoAvailable" : "saldoEscrowed";
    ...
    const currentBalance = target === "available" ? wallet.saldoAvailable : wallet.saldoEscrowed;
 ...
 */
export async function getTransactionHistory(walletId: string | ObjectId, limit = 50, skip = 0) {
  await connectDB();
 ...
 */
export async function getTransactionHistoryByUserId(userId: string | ObjectId, limit = 50, skip = 0) {
  await connectDB();
