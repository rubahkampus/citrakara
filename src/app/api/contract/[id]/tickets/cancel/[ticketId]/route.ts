...
import { getCancelTicketById } from "@/lib/services/ticket.service";
...
export async function GET(
  ...
  try {
    ...
    return withAuth(async (session) => {
      ...
      const ticket = await getCancelTicketById(ticketId);
      
      if (!ticket) {
        return NextResponse.json(
          { message: "Cancel ticket not found" },
          { status: 404 }
        );
      }

      const response = NextResponse.json({
        message: "Cancel ticket retrieved successfully",
        ticket,
      });
